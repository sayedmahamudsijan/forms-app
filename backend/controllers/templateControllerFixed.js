...
// Middleware to parse JSON-stringified fields
const parseJsonFields = (req, res, next) => {
  try {
    ...
    console.error('❌ Error parsing JSON fields:', {
      error: error.message,
      stack: error.stack,
      body: req.body,
...
const validateCreateTemplate = [
  ...
  body('questions')
    ...
    .custom(questions => questions.every(q => q.type !== 'select' || (Array.isArray(q.options) && q.options.length >= 2)))
    .withMessage('Select questions must have at least two options'),
        ...
        include: [
          {
            ...
            include: [
              { 
                ...
                as: 'TemplateQuestion', 
                attributes: ['id', 'title', 'is_visible_in_results', 'attachment_url', 'type', 'options'], 
